...
async function validateCollections(slugs) {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      // v3 collections can return either an array or a wrapped object (e.g. { data: [...], meta: {} })
      // As long as there is no error and data is present, treat the slug as valid.
      if (result.data === null || result.data === undefined) {
        logger.warn(`Collection "${slug}" returned no data. Skipping.`);
        continue;
      }
